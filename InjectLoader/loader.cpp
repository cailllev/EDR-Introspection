...
int main(int argc, char* argv[]) {
    ...
    std::string usage = "";
    usage += "[*] InjectLoader: Usage: " + exeName + " <Target PID> <FindProcHandle> <DLL Path> <(L)oadLibrary | (H)ostMapped | (R)eflective | (S)top> <(C)reateRemoteThread | (H)ijackThread | (Q)ueueUserAPC> [RiskInstability] [Debug]\n";
    ...
    if (_stricmp(argv[4], "S") == 0 || _stricmp(argv[4], "stop") == 0) {
    ...
    if (_stricmp(argv[4], "L") == 0 || _stricmp(argv[4], "loadlibrary") == 0) {
        injectType = LOADLIBRARY_INJECTION;
    }
    else if (_stricmp(argv[4], "H") == 0 || _stricmp(argv[4], "hostmapped") == 0) {
        injectType = HOSTMAPPED_INJECTION;
    }
    else if (_stricmp(argv[4], "R") == 0 || _stricmp(argv[4], "reflective") == 0) {
        injectType = REFLECTIVE_INJECTION;
    }
    else {
        std::cerr << "[!] InjectLoader: InjectType '" << argv[4] << " 'not defined.\n";
        return 1;
    }

    Executor execType;
    if (_stricmp(argv[5], "C") == 0 || _stricmp(argv[5], "createremotethread") == 0) {
        execType = CREATE_REMOTE_THREAD;
    }
    else if (_stricmp(argv[5], "H") == 0 || _stricmp(argv[5], "hijackthread") == 0) {
		execType = HIJACK_THREAD;
	}
	else if (_stricmp(argv[5], "Q") == 0 || _stricmp(argv[5], "queueuserapc") == 0) {
    ...
    HANDLE hProc = NULL;
    if (findProcHandle) {
        std::cout << "[*] InjectLoader: Process handle should already exist in this injector process -> searching for it...\n";
