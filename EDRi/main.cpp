...
int main(int argc, char* argv[]) {
    ...
    options.add_options()
        ...
        ("r,run-as-child", "Execute the attack automatically as a child-proc (default manual)")
        ("o,output-path-custom", "Writing to " + get_output_path("[name]", true), cxxopts::value<std::string>())
        ("m,trace-etw-misc", "Trace misc ETW (default no)")
    ...
    if (hook_ntdll) {
        ...
        for (auto& proc : procs_to_be_hooked) {
            ...
            if (!InjectDll(pid, get_hook_dll_path(edr_profile.needs_minimal_hooks), g_debug, Action::REFLECTIVE_INJECTION, NULL, false)) {
