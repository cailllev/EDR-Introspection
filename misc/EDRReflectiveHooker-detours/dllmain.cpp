...
bool InstallHooks() {
    ...
    if (!g_origNtQueryInformationProcess) {
        emit_etw_error("NtQueryInformationProcess not found");
        return false;
    }

    if (DetourTransactionBegin() != NO_ERROR) {
        emit_etw_error("DetourTransactionBegin failed");
        return false;
    }

    if (DetourUpdateThread(GetCurrentThread()) != NO_ERROR) {
        emit_etw_error("DetourUpdateThread failed");
    ...
    for (auto& h : funcs) {
        FARPROC target = GetProcAddress(hNtdll, h.name);
        if (!target) {
            emit_etw_error(std::string(h.name) + " not found in ntdll");
            DetourTransactionAbort();
            return false;
        }

        *h.original = target;
        if (DetourAttach(h.original, h.hook) != NO_ERROR) {
            emit_etw_error(std::string("Failed to hook ") + h.name);
            DetourTransactionAbort();
            return false;
        }
        else {
            emit_etw_ok(std::string("Hooked ") + h.name);
        }
    }

    if (DetourTransactionCommit() != NO_ERROR) {
        emit_etw_error("DetourTransactionCommit failed");
        return false;
    }
...
void RemoveHooks() {
    std::cout << "[+] Hook-DLL: Removing hooks...\n";
    ...
    for (auto& h : funcs) {/*
        if (DetourDetach(h.original, h.hook) != NO_ERROR) {
            emit_etw_error(std::string("Failed to unhook ") + h.name);
            DetourTransactionAbort();
            return;
        }
        else {
            emit_etw_ok(std::string("Unhooked ") + h.name);
        }
    */}
